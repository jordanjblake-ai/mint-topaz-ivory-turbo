            ...
            {coaches.map((coach, index) => (
              ...
              >
                {coach.image ? (
                  ...
                  <div
                    className={`flex aspect-4/5 w-full items-end rounded-lg bg-surface p-8 shadow-border ${index % 2 === 1 ? "lg:order-2" : ""}`}
                  >
                    <p className="font-display text-8xl text-accent">
                      {coach.name
                        .split(" ")
                        .map((part) => part[0])
                        .join("")}
